...
def init_research_fields(db: Session):
    # 创建顶级领域：Computer Science
    computer_science = ResearchField(
        short_name="cs",
        name="Computer Science",
        description="Computer Science and its related fields",
        level=1,
    )
    db.add(computer_science)
    db.flush()  # 获取computer_science的id
    ...
    # 批量创建子领域
    for field_data in sub_fields:
        field = ResearchField(
            short_name=field_data["short_name"],
            name=field_data["name"],
            description=field_data["description"],
            parent_id=computer_science.id,
            level=2,
        )
        db.add(field)
